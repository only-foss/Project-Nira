...
2. **Offline Logging**: A fallback micro-SD card writes a `nira_log.csv` file natively to prevent data loss.
3. **Internal Temperature Compensation**: A linear drift algorithm eliminates standard environmental variance (`comp_diff = diff - (temp_c - 25.0) * 8.5`).
